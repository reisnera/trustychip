...
type FrameAudioBuffer = AudioBuffer<{ AUDIO_FRAMES_PER_VIDEO_FRAME * 2 }>;

// fn generate_audio_sample_batch() -> &'static [i16] {
fn generate_audio_sample_batch() -> MutexGuard<'static, Box<FrameAudioBuffer>> {
    static AUDIO_BUFFER: Lazy<Mutex<Box<FrameAudioBuffer>>> =
...
pub fn run() {
    ...
    // let len = BUZZER_SAMPLES.as_slice().len();
    // assert_eq!(len, AUDIO_FRAMES_PER_VIDEO_FRAME * 2);
    // cb::audio_sample_batch(BUZZER_SAMPLES.as_slice());
